from quickumls import get_quickumls_client,QuickUMLS
import http.server
import socketserver
...
class UMLSHandler(http.server.SimpleHTTPRequestHandler):
    def sendAccessHeaders(self):
    ...
    def do_GET(self):
        try:
            ...
            self.end_headers()
            self.wfile.write(bytes(json.dumps(result, ensure_ascii=True), encoding='utf-8'))
    ...
    def processText(self, text):
        try:
            if (text != ''):
                ...
                matcher.accepted_semtypes = MY_SEMTYPES
                result = matcher.match(text, best_match=False, ignore_syntax=True)  
                json_bytes = bytes(json.dumps(result, default=set_default, ensure_ascii=True), encoding='utf-8')
    ...
    def processRequest(self, headers):
        ...
        if (content_type == "application/json"):
            try:
                ...
                result = { "error": "Received plain text data." }
                self.send_response(400)
                self.sendAccessHeaders()
                self.send_header("Content-type", "application/json")
                self.end_headers()
...
with socketserver.TCPServer(("", PORT), UMLSHandler) as httpd:
